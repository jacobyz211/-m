...
// CORS: allow Eclipse + DebridMusic
app.use(
  '*',
  cors({
    origin: '*',
    allowMethods: ['GET', 'POST', 'OPTIONS'],
    allowHeaders: ['Content-Type', 'x-client-platform'],
  })
);
...
function rememberTrack(t) {
  ...
  const isrc =
    (t.publisher_metadata && t.publisher_metadata.isrc) || t.isrc || null;
...
function effectiveCid(entry, env) {
  return entry && entry.clientId
    ? entry.clientId
    : env.SC_CLIENT_ID || SHARED_CLIENT_ID;
}
...
async function redisCmd(env, ...args) {
  ...
  try {
    const r = await fetch(
      env.REDIS_URL + '/' + args.map(encodeURIComponent).join('/'),
      {
...
function checkRateLimit(entry) {
  const now = Date.now();
  entry.rateWin = (entry.rateWin || []).filter(
...
async function tryExtract() {
  for (const pu of [
    ...
    // 1) Some client_id leaks are inline in <script> tags — check first (cheap).
    ...
    // 2) FIX: SoundCloud now ships client_id inside external JS bundles
    // (e.g. https://a-v2.sndcdn.com/assets/xxxx.js). Extract those <script src="...">
    // URLs from the HTML and scan each bundle for the client_id pattern.
...
async function fetchSharedClientId(env) {
  if (env.SC_CLIENT_ID) {
    SHARED_CLIENT_ID = env.SC_CLIENT_ID;
    return;
  }
...
async function resolveStubs(cid, tracks, fbArt, oauthToken) {
  ...
  return tracks
    .map((t) => map[String(t.id)] || t)
    .map((t) => (t.title ? t : { ...t, title: 'Unknown Track' }));
}
...
async function deezerFindBestTrack(title, artist, isrc, env) {
  ...
  if (isrc) {
    try {
      const data = await deezerApi(env, '/search', {
        q: 'isrc:"' + isrc + '"',
      });
      const items = (data && data.data) || [];
      const match = items.find(
        (t) => t.isrc && t.isrc.toUpperCase() === isrc.toUpperCase()
      );
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{background:#0f0f0f;color:#e8e8e8;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;min-height:100vh;display:flex;justify-content:center;padding:40px 16px}
  .wrap{max-width:560px;width:100%}
  .card{background:#161616;border:1px solid #232323;border-radius:18px;padding:28px 24px 24px;box-shadow:0 24px 64px rgba(0,0,0,.5)}
  h1{font-size:22px;font-weight:700;margin-bottom:6px}
  .sub{font-size:14px;color:#888;line-height:1.6;margin-bottom:18px}
  .lbl{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:#555;margin-top:16px;margin-bottom:7px}
  input{width:100%;background:#101010;border:1px solid #252525;border-radius:10px;color:#e8e8e8;font-size:14px;padding:11px 12px;outline:none;margin-bottom:6px;transition:border-color .15s}
  ...
  .btn{display:inline-block;width:100%;margin-top:14px;border-radius:10px;border:none;padding:12px;font-size:15px;font-weight:700;cursor:pointer;transition:background .15s,transform .05s}
  .btn-primary{background:#f50;color:#fff}
  .btn-primary:hover{background:#d94a00;transform:translateY(-1px)}
  .btn-primary:disabled{background:#252525;color:#555;cursor:not-allowed;transform:none}
  .url-box{display:none;margin-top:14px;border-radius:12px;border:1px solid #252525;background:#101010;padding:12px 12px 10px}
  .url-label{font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:#555;margin-bottom:6px}
  .url-value{font-size:12px;font-family:"SF Mono","Menlo",monospace;color:#f50;word-break:break-all;line-height:1.5}
  .url-actions{margin-top:10px;display:flex;gap:8px}
  .btn-secondary{flex:1;background:#1a1a1a;color:#ccc;border:1px solid #2a2a2a;font-size:13px}
  ...
  .steps{margin-top:18px;border-top:1px solid #222;padding-top:16px;font-size:13px;color:#777}
...
<div class="wrap">
  <div class="card">
    ...
    <p class="sub">
      Uses SoundCloud for search and playback. If SoundCloud marks a track as a 30-second preview (policy SNIP),
      this addon tries Deezer (via ARL) to find a full stream instead.
    </p>
...
app.post('/generate', async (c) => {
  ...
  if (bucket.count >= MAX_TOKENS_PER_IP)
    return c.json({ error: 'Too many tokens today from this IP.' }, 429);
  ...
  return c.json({
    token,
    manifestUrl: getBaseUrl(c) + '/u/' + token + '/manifest.json',
  });
});
...
app.get('/u/:token/manifest.json', async (c) =>
  withToken(c, () =>
    c.json({
      ...
      version: '1.0.0',
      description:
        'SoundCloud streams with Deezer fallback for 30-second previews.',
...
app.get('/u/:token/search', async (c) =>
  withToken(c, async (entry) => {
    ...
    if (!SHARED_CLIENT_ID && !c.env.SC_CLIENT_ID) {
      // FIX: cap the wait so a slow/failed scrape never hangs the request.
      await Promise.race([
        fetchSharedClientId(c.env).catch(() => {}),
        sleep(4000),
      ]);
    }
    ...
    try {
      ...
      return c.json(
        { error: 'Search failed.', tracks: [], albums: [], artists: [], playlists: [] },
        500
      );
    }
  })
    ...
    return dedupeCall('stream:' + tid, async () => {
      ...
      try {
        try {
          track = await scGet(
            cid,
            'https://api-v2.soundcloud.com/tracks/soundcloud:tracks:' + tid,
            {},
            scOAuth(entry)
          );
        } catch {
          track = await scGet(
            cid,
            'https://api-v2.soundcloud.com/tracks/' + tid,
            {},
            scOAuth(entry)
          );
        }
      ...
      async function resolveTranscoding(transcoding, preview) {
        ...
        const mime =
          (transcoding.format && transcoding.format.mime_type) || '';
      ...
      if (isSnippet) {
        ...
        return c.json(
          { error: 'No full stream found for snippet track ' + tid },
          404
        );
      }

      if (liveTc.length > 0) {
        const progressives = liveTc.filter(
        ...
        return c.json(
          { error: 'SoundCloud stream unavailable for track ' + tid },
          404
        );
      }
